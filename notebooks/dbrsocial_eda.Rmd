...
query <- "SELECT  year, new_id, count(distinct cve_muni) as num_muni, count(distinct cve_programa) as numero_programas, sum(nu_imp_monetario) as monto_anual
              ...
              ORDER BY num_muni DESC LIMIT 50;"
monto_persona_muni <- load_or_run(con,query,los_queries)
```


```{r, echo=FALSE}
monto_persona_ent <- dbGetQuery(con,
          "SELECT  year, new_id, count(distinct cve_ent) as num_ent, count(distinct cve_programa) as numero_programas, sum(nu_imp_monetario) as monto_anual, cd_beneficio
              FROM pub_nominal.nominal
              GROUP BY year, new_id, cve_padron, cd_beneficio
...
```{r, echo=FALSE}
monto_emprendedor <- dbGetQuery(con,
          "SELECT  year, sum(nu_imp_monetario) as monto_anual, cd_beneficio, cve_programa
...
```{r}
monto_emprendedor %>% View()
```
query <- "SELECT * FROM pub_nominal.nominal tablesample bernoulli(0.001) repeatable(1234)"
